var rule = {
  ...
  搜索: $js.toString(() => {
    ...
    let d = [];
    // 真实壳选择器行为不可信，直接用正则扒 HTML：每个结果 = div.thumb + div.detail
    let re = /<div class="thumb">([\s\S]*?)<\/div>\s*<div class="detail">([\s\S]*?)<\/div>/g;
    let m;
    while ((m = re.exec(html)) !== null) {
      let thumb = m[1];
      let detail = m[2];
      // 真片名：div.detail > h4.title > a.searchkey 文本
  ...
  二级: $js.toString(() => {
    ...
    let pic = pd(html, 'a[data-original]&&data-original') || pd(html, 'a&&data-original') || pd(html, 'img[data-original]&&data-original') || pd(html, 'img&&src') || ''; if (!pic) { let st = pdfh(html, 'a&&style'); let sm = st.match(/url\(["']?([^)"']+)/); pic = sm ? sm[1] : ''; }
    ...
    let froms = [], urls = [];
    Object.keys(lines).sort(function(a,b){return a-b;}).forEach(function(k, idx){
      froms.push('君子兰' + (idx+1));
      urls.push(lines[k].join('#'));
    });
    VOD = { vod_name: name, vod_pic: pic, vod_play_from: froms.join('$$$'), vod_play_url: urls.join('$$$') };
  }),
  lazy: $js.toString(() => {
    let html = fetch(input, fetch_params);
