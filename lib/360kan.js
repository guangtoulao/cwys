...
// 注意：真机引擎把「推荐/一级/搜索」等 js 体放在独立作用域 eval，
// 本文件顶层挂的 globalThis.* 在 js 体里可能不可见，会导致 request(API+...) 拿到 undefined。
// 因此 API / SIGN / jsonOf 一律**内联**进每个 js 体使用，不依赖顶层全局。
// （request / setResult / buildUrl / urlDeal / KEY / MY_CATE / MY_PAGE 是引擎内置，可直接用）
...
var rule = {
  ...
  推荐: $js.toString(() => {
    ...
    var API = 'https://api.web.360kan.com/v1';
    var SIGN = 'sign=360_146029f4&device=2&net=5&callback=cb';
    var cats = [1, 2, 3, 4]; // 电影/电视剧/综艺/动漫
    var seen = {};
    var list = [];
    for (var c = 0; c < cats.length; c++) {
      var res = '';
      try { res = request(API + '/rank?' + SIGN + '&cat=' + cats[c]); } catch (e) { res = ''; }
      var arr = _json(res).data || [];
  ...
  一级: $js.toString(() => {
    ...
    var SIGN = 'sign=360_146029f4&device=2&net=5&callback=cb';
    var res = '';
    try { res = request(API + '/rank?' + SIGN + '&cat=' + MY_CATE); } catch (e) { res = ''; }
    var arr = _json(res).data || [];
    var list = [];
    for (var i = 0; i < arr.length; i++) {
      var it = arr[i];
  ...
  搜索: $js.toString(() => {
    ...
    var rows = {};
    try { rows = _json(request(api, { headers: hd })).data || {}; } catch (e) { rows = {}; }
  ...
  // buildUrl / urlDeal 是引擎内置函数，可直接用；jsonOf 内联避免依赖顶层全局
  二级: $js.toString(() => {
    function _json(s) {
      s = String(s || '').trim();
      try { return JSON.parse(s); } catch (e) {}
    ...
    var data = {};
    try { data = _json(request(input)).data || {}; } catch (e) { data = {}; }
    var vod_play = {};
    ...
    for (var si = 0; si < sites.length; si++) {
      ...
      if (total > 1) {
        ...
        for (var j = 1; j < total; j += delta) {
          ...
          var url2 = '';
          try { url2 = buildUrl(input, { start: j, end: end, site: site }); } catch (e) { url2 = ''; }
          var vd = {};
          try { vd = _json(request(url2)).data || {}; } catch (e) { vd = {}; }
          if (!vd) continue;
          var eps = (vd.allepidetail && vd.allepidetail[site]) ? vd.allepidetail[site] : (vd.defaultepisode || []);
          for (var k = 0; k < eps.length; k++) {
            var epurl = '';
            try { epurl = urlDeal(eps[k].url || ''); } catch (e) { epurl = eps[k].url || ''; }
            vodItems.push((eps[k].playlink_num || eps[k].period || '') + '$' + epurl);
          }
        }
      } else {
        ...
        if (item && item.default_url) {
          var du = '';
          try { du = urlDeal(item.default_url); } catch (e) { du = item.default_url; }
          vodItems.push((item.sort || '') + '$' + du);
        }
      }
