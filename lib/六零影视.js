var rule = {
  ...
  url: "/fyclass/",
  searchUrl: "https://spro.so.com/searchthrow/api/midpage/throw??ls=**&amp;q=**&amp;lmsid=**&amp;lmid=**&amp;mid=**&amp;huid=**&amp;ctype=**&amp;rurl=**&amp;bucket_id=**&amp;ism=**&amp;seq=**&amp;travel_city=**&amp;index=**&amp;lm_extend=**",
  searchable: 2,
  headers: { 'User-Agent': 'Mozilla/5.0 (Linux; Android 11; M2007J3SC Build/SKQ1.220303.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/111.0.5563.116 Mobile Safari/537.36', 'Referer': "https://www.360kan.com" },
  timeout: 10000,
  play_parse: true,
  class_name: "电视剧&综艺&电影&儿童&动漫&经典",
  class_url: "dianshi&zongyi&dianying&dongman&jingdian",
  // 推荐/一级/搜索：列表;标题;图片;描述;链接（选择器串，工具自动识别）
  推荐: "a.link;&&Text;img&&src;;&&href",
  一级: "a.link;&&Text;img&&src;;&&href",
  搜索: "li;a&&Text;img&&src;;a&&href",
  ...
  if (links && links.length) {
    for (var i = 0; i < links.length; i++) {
      ...
      if (lk.indexOf('http') !== 0) lk = "https://www.360kan.com" + lk;
  ...
  if (uls && uls.length >= 1 && (uls.length > 1 || (tabs && tabs.length > 1))) {
    for (var ui = 0; ui < uls.length; ui++) {
      ...
      for (var li2 = 0; li2 < lis.length; li2++) {
        ...
        if (lk) { if (lk.indexOf('http') !== 0) lk = "https://www.360kan.com" + lk; eps.push(epName + '$' + lk); }
      }
  ...
  if (!Object.keys(srcMap).length && !froms.length) {
    ...
    for (var j = 0; j < all.length; j++) {
      ...
      if (u.indexOf('http') !== 0) u = "https://www.360kan.com" + u;
