...
var rule = {
  ...
  a99make: function (C, P, RQ, R) {
    ...
    function buildDetail(det) {
      ...
      // 直播（play_url 走 live.php 代理）：线路名用播放器配置的真实名称，与原版一致（CCTV→央视直播、difangtai→地方台直播）
      // 点播：线路名仍用「前缀+带圈数字」（君子兰①…）
      ...
      for (var i2 = 0; i2 < pu.length; i2++) {
        ...
        for (var j = 0; j < segs.length; j++) {
          // 原始条目统一是「标签$地址」：电影标签=集号(1/2/3…)；直播标签=频道名(CCTV1综合/浙江卫视…)
          // 旧写法只匹配「纯数字$」→ 直播名里夹数字(CCTV1综合)直接不匹配、名字被丢成兜底"1"，全部按钮变 1。
