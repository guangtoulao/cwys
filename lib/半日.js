// 半日(App99 / bn) —— drpy2 var rule
// 协议：AES-CBC(随机32hex uuid 密钥,IV前置) + sha256 签名 + zlib 响应
// 从 spider.jar 的 App99.smali 逆向还原，真引擎+真接口验证通过。
// 部署：api 走你壳的 drpy2 加载器（如 type:3 / api:"csp_js"），ext 填配置（默认值已内置，可覆盖）。
var rule = {
  title: "半日",
  ...
  play_parse: true,
  timeout: 15000,
  // 分类（首次运行由系统初始化填充；此处给真实 id 兜底，避免首页空白）
  class_name: "电视剧&电影&动漫&综艺&短剧&少儿&纪录片&直播",
  class_url: "2&1&3&4&22&20&5&23",
  // ===== 站点配置（与用户 api.json 的半日99 一致，可被 ext 覆盖）=====
  versionName: "3.5.8",
  name: "半日闲",
  ...
  // 线路显示名前缀与带圈数字
  linePrefix: "半日",
  // ===== 协议核心（工厂函数，闭包安全：C/P/RQ 来自引擎作用域，rule 来自引擎变量）=====
  a99make: function (C, P, RQ, R) {
    ...
    function enc(keyWA, bodyStr) {
      var ivWA = C.lib.WordArray.random(16);
    ...
    function nonce() { return C.enc.Base64.stringify(C.lib.WordArray.random(16)); }
    function post(path, obj) {
      ...
      // 同时给 body 和 data：不同壳的 request 读 POST 体用的 key 不同
      var _r = RQ(R.host + path, { method: "POST", headers: headers, body: be, data: be });
      // 防御式取值：request 返回形态跨引擎不一致（字符串 / {content} / {body} / {data}）
      var resp = _r;
    ...
    function buildDetail(det) {
      ...
      for (var i = 0; i < pf.length; i++) {
        var code = pf[i] || ("c" + i);
        var rn = R.linePrefix + (CIRCLED[i] || String(i + 1));
        while (map[rn]) rn += "②";
      ...
      for (var i2 = 0; i2 < pu.length; i2++) {
        ...
        for (var j = 0; j < segs.length; j++) {
          var m = /^\s*(\D*)(\d*)\$(.+)$/.exec(segs[j]); if (!m) continue;
          var epnum = m[2] || "1"; var u = m[3];
          eps.push(epnum + "$" + u + "@" + code2 + "@" + (det.name || "") + "@" + epnum);
        }
        lines.push(eps.join("#"));
      }
      return {
