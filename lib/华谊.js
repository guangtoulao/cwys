...
function _dramaList(top){var out=[];try{var d=_pbGet(top,3);if(!d)return out;
  var its=_pf(d.v);for(var i=0;i<its.length;i++){if(its[i].f!==1||its[i].w!==2)continue;
    ...
    out.push({url:String(typeof m[3]==="number"?m[3]:_utf8(m[3]||[])),title:_utf8(m[5]||[]),pic_url:pic,desc:_utf8(m[13]||[])});}
  }catch(e){}return out;}
...
(Array.isArray(data)?data:[]).forEach(function(sec){var secs=(sec.sections&&sec.sections.length)?sec.sections:[sec];
  secs.forEach(function(ss){(ss.vodList||[]).forEach(function(v){
    list.push({url:String(v.id),title:v.name,pic_url:(v.coverImage&&v.coverImage.path)||"",desc:v.remarks||""});});});});
}catch(e){list=[{url:"err",title:"ERR推荐:"+e.message}];}
setResult(list);
...
}catch(e){setResult([{url:"err",title:"ERR一级:"+e.message}]);}
...
var _MEDIA=/(.*)\.(mp4|m3u8|flv|mkv|avi|ts|mov|mpd|m4a|wmv)(\?.*)?$/i;
var id=String(input).split("?")[0].split("/").pop()||String(input);
try{
...
vids.forEach(function(vb){var vm={},vf=_pf(vb),j;for(j=0;j<vf.length;j++)vm[vf[j].f]=(vf[j].w===2)?_utf8(vf[j].v):Number(vf[j].v);
  ...
  else{lines[line].push(title+"$"+(vm[9]||"")+"@@@"+path);}});
...
}catch(e){setResult([{url:"err",title:"ERR搜索:"+e.message}]);}
...
var raw=String(input);var id=raw.indexOf("$")>=0?raw.split("$").slice(1).join("$"):raw;
var _MEDIA=/(.*)\.(mp4|m3u8|flv|mkv|avi|ts|mov|mpd|m4a|wmv)(\?.*)?$/i;
var _HD={"User-Agent":"okhttp/3.12.1"};
if(_MEDIA.test(id)){input={parse:0,jx:0,url:id,header:_HD};}
else{var pp=id.split("@@@"),vp=pp[0]||"",pu=pp[1]||"",url="";
  try{var top=_postPb("/api/proto/v5/videoUsableUrl",{vodPlayFrom:vp,playUrl:pu});
  var d=_pbGet(top,3);if(d){var u=_pbGet(_pf(d.v),1);if(u)url=_utf8(u.v);}}catch(e0){}
  input={parse:0,jx:0,url:url,header:_HD};}
